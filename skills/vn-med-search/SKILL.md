...
description: >-
  ...
  Tạp chí Khoa học VNU (VNU JS), Thư viện Quốc gia (NLV), VISTA/STI,
  Tạp chí Y Dược Huế / Journal of Medicine and Pharmacy Hue (JMP / HueJMP),
  Tạp chí Y học TP.HCM / Ho Chi Minh City Journal of Medicine (TCYHTPHCM),
  MedPharmRes (Medical and Pharmaceutical Research),
  Tạp chí Y học Dự phòng (VJPM),
  hoặc Tạp chí Y Dược học Cần Thơ (CTUMP).
---
...
- **JMP vs HueJMP**: `jmp` (jmp.huemed-univ.edu.vn) chỉ lưu trữ bài từ 2011-2023 (site PHP cũ). Bài từ 2023 trở đi nằm ở `huejmp` (huejmp.vn, OJS mới). Nên dùng cả hai: `--sources jmp,huejmp`.
- **tcyhtphcm**: Tạp chí Y học TP.HCM được định danh qua mã DOI prefix `10.32895` trên hệ thống Crossref quốc tế (hơn 1.100 bài báo). Adapter kết nối trực tiếp với Crossref API để tìm kiếm chính xác, sau đó crawl trang bài viết trên `tapchiyhoctphcm.vn` để trích xuất tóm tắt và link PDF gốc.
- **MedPharmRes**: Tạp chí tiếng Anh, search trả về kết quả tiếng Anh. Cùng publisher với `tcyhtphcm` (Đại học Y Dược TP.HCM). Indexed in DOAJ. Query tiếng Anh cho kết quả tốt hơn.
- **vjpm & ctump**: Chạy hệ thống OJS chuẩn với đầy đủ metadata Google Scholar (citation_title, citation_author, citation_pdf_url, abstract song ngữ).
...
5. **Bước 5** *(có thể bỏ qua)*: Hỏi: *"Nguồn ưu tiên? (tcncyh / vmj / bv108 / vnu / nlv-luanan / vista / jmp / huejmp / tcyhtphcm / medpharmres / vjpm / ctump, hoặc 'all')"*
...
# Tìm trên 3 nguồn mới
uv run ... vn_med_search.py "ung thư gan" \
  --sources jmp,huejmp,tcyhtphcm,medpharmres --json

# Danh sách tất cả source ID hợp lệ:
# tcncyh   — Tạp chí Nghiên cứu Y học
# vmj      — Tạp chí Y học Việt Nam
# bv108    — Tạp chí Y Dược Lâm sàng 108
# vnu      — Tạp chí Khoa học VNU (Y Dược)
# nlv-luanan — Thư viện Quốc gia (Luận án)
# vista    — VISTA STI
# jmp      — Tạp chí Y Dược Huế cũ (2011-2023, jmp.huemed-univ.edu.vn)
# huejmp   — Tạp chí Y Dược Huế mới (từ 2023, huejmp.vn)
# tcyhtphcm — Tạp chí Y học TP.HCM (Crossref API + tapchiyhoctphcm.vn)
# medpharmres — MedPharmRes (medpharmres.com, tiếng Anh, DOAJ indexed)
# vjpm     — Tạp chí Y học Dự phòng (vjpm.vn)
# ctump    — Tạp chí Y Dược học Cần Thơ (tapchi.ctump.edu.vn)
```
